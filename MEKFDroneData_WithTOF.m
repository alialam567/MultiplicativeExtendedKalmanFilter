...
for i = 1:length(t)
    ...
    particlefilter = kf.position;

    pos_meas = particefilter.position;

    % MEKF update
    acc_meas_corrected = accelFiltered;
    kf.update(gyroData(i,:)', acc_meas_corrected(i,:)', tof_data', dt);
...
for i = 1:3
    subplot(3,1,i);
    plot(t, eul_gyro(:,i), 'r', 'LineWidth', 1.2); hold on;
    plot(t, eul_est(:,i), 'b', 'LineWidth', 1.2);
    plot(gt_t, eul_true(1:length(gt_t),i), 'k--', 'LineWidth', 1.2); % Ground truth
    ylabel([labels{i} ' (deg)']);
    grid on;
    if i == 1
