...
initialPos = [1 0 2];
initialVel = [0 1.2566 0];
initialCov = 1e-5;

% Create MEKF
kf = MEKF_TOF(initialQuat, initialPos, initialVel,initialCov, ...
    1e-5, 1e-5, 1e-3, 1e-8, 0.1, 1e-5, 1e-5, 0.1);
...
[ax,ay,az,gx,gy,gz,p1,p2,p3] = load_sam_data('drone_sim_log_noise.mat');

accelData = zeros(length(t),3);
gyroData = zeros(length(t),3);
tofData = zeros(length(t),3);

accelData = [ax(1:length(t)), ay(1:length(t)), az(1:length(t))];
gyroData  = [gx(1:length(t)), gy(1:length(t)), gz(1:length(t))];
tofData  = [p1(1:length(t)), p2(1:length(t)), p3(1:length(t))];

% Run MEKF, gyro-only estimation
for i = 1:length(t)
    ...
    % MEKF update
    kf.update(gyroData(i,:)', accelData(i,:)', tofData(i,:)',  dt);
...
title('h\_predicted'); legend('x', 'y', 'z'); ylabel('Value');

subplot(3,1,2);
plot(time, kf.acc_meas_norm_log');
title('acc\_meas\_norm'); legend('x', 'y', 'z'); ylabel('Value');

subplot(3,1,3);
plot(time, kf.delta_theta_log');
title('\delta\theta'); legend('\delta\theta_x', '\delta\theta_y', '\delta\theta_z');
...
function [ax,ay,az,gx,gy,gz,px,py,pz] = load_sam_data(filename)
    load(filename, "data_log_table")  % Load table
    
    % Extract data using table column names
    ax = data_log_table.accel_x;
    ay = data_log_table.accel_y;
    az = data_log_table.accel_z;
    
    gx = data_log_table.gyro_x;
    gy = data_log_table.gyro_y;
    gz = data_log_table.gyro_z;

    px = data_log_table.x;  % Assuming these are the correct column names
    py = data_log_table.y;
    pz = data_log_table.z;
end
